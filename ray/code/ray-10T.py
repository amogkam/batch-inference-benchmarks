...
ds = ray.data.read_parquet("s3://air-example-data-2/10T-image-data-synthetic-raw-parquet/")
...
ds = ds.map_batches(Actor, batch_size=BATCH_SIZE, compute=ActorPoolStrategy(size=40), num_gpus=1, batch_format="numpy", fn_constructor_kwargs={"model": model_ref}, max_concurrency=2)
for _ in ds.iter_batches(batch_size=None, batch_format="pyarrow"):
    pass
end_time = time.time()

print("Total time: ", end_time-start_time)
print("Throughput (img/sec): ", (16232000)/(end_time-start_time))
print("Total time w/o metadata fetching (img/sec) : ", (end_time-start_time_without_metadata_fetching))
print("Throughput w/o metadata fetching (img/sec) ", (16232000)/(end_time-start_time_without_metadata_fetching))
